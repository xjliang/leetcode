...
class Solution {
 public:
    ...
    backtrack(result, track, candidates, 0, target, 0);
    return result;
  }

  void backtrack(std::vector<std::vector<int>>& result, std::vector<int>& track,
                 const std::vector<int>& candidates, int curr,
                 int target, int sum) {
    if (sum == target) {
      result.push_back(track);
      return ;
    }
    if (curr == candidates.size() || sum > target) {
      return ;
    }

    track.push_back(candidates[curr]);
    backtrack(result, track, candidates, curr, target, sum + candidates[curr]);
    track.pop_back();

    backtrack(result, track, candidates, curr + 1, target, sum);
  }
};
