...
class Solution {
 public:
  int maxArea(const std::vector<int>& height) {
    ...
    while (left < right) {
      result = std::max(result,  area(height, left, right));
 ...
 private:  
  static int area(const std::vector<int>& nums, int left, int right) {
    return std::min(nums[left], nums[right]) * (right - left);
  }
};
